...
from .callbacks import FloydhubLoggerCallback, MAPCallback
...
# These files should be hosted on server ####
...
class YOLO(object):
    # Feature nets
    FEATURE_NETS = {
        'Full Yolo': FullYoloFeature,
        'Tiny Yolo': TinyYoloFeature,
        'MobileNet': MobileNetFeature,
    }
    DEFAULT_PATHS = {
        'Full Yolo': FULL_YOLO_BACKEND_PATH,
        'Tiny Yolo': TINY_YOLO_BACKEND_PATH,
        'MobileNet': MOBILENET_BACKEND_PATH,
    }
    ...
    def __init__(self, backend,
        ...
        self.feature_extractor = self._load_backend(
            backend, input_size, weight_path)
        logger.info("Feature net output shape %s",
                    str(self.feature_extractor.get_output_shape()))
    ...
    def _load_backend(self, backend, input_size, weight_path):
        if backend not in self.FEATURE_NETS:
            raise Exception(
                'Architecture not supported! Only support Full Yolo,'
                'Tiny Yolo, MobileNet, SqueezeNet, VGG16, ResNet50, '
                'and Inception3 at the moment!')
        if weight_path is None:
            weight_path = self.DEFAULT_PATHS[backend]
        return self.FEATURE_NETS[backend](input_size, weight_path)

    def custom_loss(self, y_true, y_pred):
        mask_shape = tf.shape(y_true)[:4]

        cell_x = tf.to_float(
            tf.reshape(tf.tile(tf.range(self.grid_w), [
        ...
        # current_recall = tf.Variable(0., name="current_recall")
        ...
        Determine the masks
        """
        # coordinate mask: simply the position
        # of the ground truth boxes (the predictors)
        coord_mask = tf.expand_dims(y_true[..., 4], axis=-1) * self.coord_scale

        # confidence mask: penelize predictors + penalize boxes with low IOU
        # penalize the confidence of the boxes,
        # which have IOU with some ground truth box < 0.6
        ...
        # penalize the confidence of the boxes,
        # which are reponsible for corresponding ground truth box
        conf_mask = conf_mask + y_true[..., 4] * self.object_scale

        # class mask: simply the position of
        # the ground truth boxes (the predictors)
        class_mask = y_true[..., 4] * \
        ...
        loss_xy = tf.reduce_sum(
            tf.square(
                true_box_xy-pred_box_xy) * coord_mask) / (
                    nb_coord_box + 1e-6) / 2.
        ...
        if self.debug:
            ...
            nb_slightly_pred_box = tf.reduce_sum(tf.to_float(
                true_box_conf > 0.5) * tf.to_float(pred_box_conf > 0.1))
            # print(current_recall)
            # current_recall = tf.assign(
            # current_recall, nb_pred_box/(nb_true_box + 1e-6))
            # print("After assign")
            current_recall = nb_pred_box/(nb_true_box + 1e-6)
            slight_current_recall = nb_slightly_pred_box/(nb_true_box + 1e-6)
            ...
            loss = tf.Print(loss, [slight_current_recall],
                            message='Relaxed Current Recall \t',
                            summarize=1000)
    ...
    def train(self, train_imgs,     # the list of images to train the model
              ...
              train_times,    # the number of time to repeat the training set,
                              # often used for small datasets
              valid_times,    # the number of times to repeat the
                              # validation set,
                              # often used for small datasets
              nb_epochs,      # number of epoches
              learning_rate,  # the learning rate
              batch_size,     # the size of the batch
              warmup_epochs,  # number of initial batches to let
                              # the model familiarize
        ...
        tensorboard = TensorBoard(log_dir=os.path.expanduser('~/logs/'),
                                  ...
                                  write_graph=True,
                                  write_images=False)
        mAP = MAPCallback(train_generator, valid_generator, self)
        logger_callback = FloydhubLoggerCallback()
        ############################################
    ...
    def evaluate(self,
        ...
        # Arguments
            ...
            score_threshold : The score confidence threshold
                            to use for detections.
            max_detections  : The maximum number of detections
                            to use per image.
            save_path       : The path to save images with
                            visualized detections to.
        # Returns
        ...
        for i in range(generator.size()):
            ...
            if len(pred_boxes) > 0:
                pred_boxes = np.array(
                    [[box.xmin*raw_width, box.ymin*raw_height, box.xmax *
                        raw_width, box.ymax*raw_height, box.score]
    ...
    def predit_generator(self, image_generator, steps=None,
                         max_queue_size=10, workers=1,
                         use_multiprocessing=False, verbose=1):
        preprocessing_generator = PreprocessingSequence(image_generator, self)
        # netout = self.model.predict_generator([input_image, dummy_array])[0]
        netouts = self.model.predict_generator(
            preprocessing_generator,
            steps=steps, max_queue_size=max_queue_size,
            workers=workers,
            use_multiprocessing=use_multiprocessing, verbose=verbose)
        boxes = (decode_netout(
            netout, self.anchors, self.nb_class) for netout in netouts)

        return boxes


class PreprocessingSequence(keras.utils.Sequence):
    """docstring for PredictorGenerator"""

    def __init__(self, wrapped, yolo):
        super(PreprocessingSequence, self).__init__()
    ...
    def __getitem__(self, index):
        image = self.wrapped[index]
        return self._prepare_image_for_prediction(image)

    def _prepare_image_for_prediction(self, image):
        image_h, image_w, _ = image.shape
        image = cv2.resize(image, (self.yolo.input_size, self.yolo.input_size))
        image = self.yolo.feature_extractor.normalize(image)

        input_image = image[:, :, ::-1]
        input_image = np.expand_dims(input_image, 0)
        dummy_array = np.zeros((1, 1, 1, 1, self.yolo.max_box_per_image, 4))
        return [input_image, dummy_array]
